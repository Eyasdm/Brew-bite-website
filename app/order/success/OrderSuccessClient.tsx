...
import type { MenuLookupEntry, OrderItemSummary, OrderRow } from "@/types/db";
...
export default function OrderSuccessClient() {
  ...
  const [items, setItems] = useState<OrderItemSummary[]>([]);
  const [menuMap, setMenuMap] = useState<Record<string, MenuLookupEntry>>({});
  ...
  useEffect(() => {
    ...
    const loadOrder = async () => {
      ...
      const map: Record<string, MenuLookupEntry> = {};
  ...
  // Narrowing guard for TS — in practice `order` is always set by the time
  // `loading` is false and `error` is null (loadOrder's success path sets
  // all three together, and React batches them into one render), but they're
  // independent useState values so TS can't infer that relationship itself.
