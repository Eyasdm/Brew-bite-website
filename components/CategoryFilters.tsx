...
function SubCategoryFilters() {
  ...
  const subCategoriesMap: Record<
    MenuCategory,
    { id: MenuSubCategory | "all"; label: string }[]
  > = {
    drink: [
      { id: "all", label: "All" },
...
function MainFilterButton({ children, active, onClick }: FilterButtonProps) {
  return (
    <button
      onClick={onClick}
      className={clsx(
...
function SubFilterButton({ children, active, onClick }: FilterButtonProps) {
  return (
    <button
      onClick={onClick}
      className={clsx(
